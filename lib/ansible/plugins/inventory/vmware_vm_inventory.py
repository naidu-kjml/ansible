...
DOCUMENTATION = '''
    ...
    options:
        ...
        with_tags:
            description:
            - Include tags associated with the given virtual machines.
            - Requires 'vSphere Automation SDK' library to be installed on the given controller machine.
            - Please refer following URL for installation steps U(https://code.vmware.com/web/sdk/65/vsphere-automation-python).
            default: False
            type: boolean
'''
...
class BaseVMwareInventory:
    ...
    def _login_vapi(self):
        ...
        client = create_vsphere_client(server=self.hostname, username=self.username, password=self.password, session=session)

        if client is None:
            self.module.fail_json(msg="Failed to login to %s using username %s" % (self.hostname, self.username))
